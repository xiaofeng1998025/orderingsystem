    ...
    <div class="Dishunit">
        <div class="Position_top">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>菜品单位</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="Position_cont">
            <div>
                <el-button type="danger" icon="el-icon-plus" size="small" @click="onnew">菜品单位新增</el-button>
            </div>
    ...
    export default {
        ...
        methods:{
            ...
            init(){
                ...
                this.Jobtitle='';
                // 原因列表
                this.$post(this.$api.attachsList1,{
                    page:this.Jopage,
                    type:'1',
                    size:this.Josize
                }).then(data=>{
                    console.log('原因列表',data);
                    if(data.code==0){
            ...
            ondetermine(){
               // console.log(this.flagID)
                if(this.type=='new'){
                    //新增
                    this.$post(this.$api.attachsAdd1,{
                        flag:this.flagID,
                        name:this.Jobtitle,
                    }).then(data=>{
                        if(data.code==0){
                    ...
                    // 修改
                    this.$post(this.$api.attachsEdit1,{attachid:this.attachid,flag:this.flagID,name:this.Jobtitle}).then(data=>{
                        if(data.code==0){
                            this.$message({
                                message: '修改成功',
                                type: 'success'
            ...
            ondelete(row){
                this.$confirm(' 是否确定删除?', '提示', {
                    ...
                    this.$post(this.$api.attachsDel1,{attachid:row.attachid}).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                type: 'success',
