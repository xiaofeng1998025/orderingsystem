    ...
    <div class="TheReason">
        <div class="Position_top">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>原因信息</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="Position_cont">
            <div>
                <el-button type="danger" icon="el-icon-plus" size="small" @click="onnew">原因新增</el-button>
            </div>
            ...
            <el-dialog :close-on-click-modal="false" :visible.sync="Modifythe"  @close="shutDown">
                <div>
                    <ul>
                        <li>
                            <p>原因：</p>
                            <el-select v-model="flag" @change="onflag"  placeholder="请选择">
                                <el-option
    ...
    export default {
        ...
        methods:{
            ...
            init(){
                ...
                this.Jobtitle='';
                // 单位列表
                this.$post(this.$api.attachsList,{
                    page:this.Jopage,
                    type:'2',
                    size:this.Josize
                }).then(data=>{
            ...
            ondetermine(){
                if(this.type=='new'){
                    ...
                    // 修改
                    this.$post(this.$api.attachsEdit,{attachid:this.attachid,flag:this.flagID,name:this.Jobtitle}).then(data=>{
                        if(data.code==0){
                            this.$message({
                                message: '修改成功',
                                type: 'success'
            ...
            ondelete(row){
                this.$confirm(' 是否确定删除?', '提示', {
                    ...
                    this.$post(this.$api.attachsDel,{attachid:row.attachid}).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                type: 'success',
