<template>
    <div class="OutboundManagement">
        <div class="Position_top">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
                <el-breadcrumb-item>仓库管理</el-breadcrumb-item>
                <el-breadcrumb-item>原料出库管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="Position_cont">
            <div>
                <el-button type="danger" icon="el-icon-plus" size="small" @click="onnew">新增出库单</el-button>
            </div>
            <div class="search">
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope" >
                            ...
                            <div v-else="scope.row.status=='2'">已取消</div>
                        </template>
                    </el-table-column>
                    <el-table-column
                            fixed="right"
                            ...
                            width="300">
                        <template slot-scope="scope">
                            <el-button type="primary" @click="onckmx(scope.row)"  size="small">出库明细</el-button>
            ...
            <el-dialog :close-on-click-modal="false" :visible.sync="Modifythe" @close="shutDown">
                <div>
                    <ul>
                        ...
                        <li>
                            <p>领料部门：</p>
                            <el-select v-model="bmxl"  clearable placeholder="请选择">
                                <el-option
                                        v-for="item in bmxlArr"
        ...
        <div class="Thepopup Thepopupmx">
            <el-dialog :close-on-click-modal="false" :visible.sync="Modifmxtc">
                <div class="Thepopupmx_top">
                    <el-button type="danger" icon="el-icon-plus" size="small" @click="onnewckyl">新增出库原料</el-button>
                    <div>
                ...
                <el-dialog
                    ...
                    <div class="Position_cont_table_body">
                        <ul style="display: flex;justify-content: space-between;flex-wrap: wrap">
                            ...
                            <li style="width: 48%;margin-top: 20px">
                                <p>总价：</p>
                                <el-input v-model="(amount*yuanlxlGg.price).toFixed(2)"  disabled placeholder="请输入内容" clearable></el-input>
                            </li>
                            <li style="width: 48%;margin-top: 20px">
                                <p>备注：</p>
    ...
    export default {
        ...
        methods:{
            ...
            yllbList(){
                ...
                this.yuanlxlGg="";
                // 原料明细列表
                this.$post(this.$api. goodschukulist,{
                    chukuid:this.chukuid,
                    page:this.mxJopage,
                    size:this.mxJosize,
                }).then(data=>{
                    console.log('出库单列表',data);
            ...
            handleClick(row) {
                ...
                this.ckxl=row.warehouseid;
                this.bmxl=row.departmentid;
                this.fzrxl=row.employee1id;
                this.memo=row.memo;
                this.chukuid=row.chukuid;
            },
            // 时间转换
            DateTime(date){
            ...
            ondetermine(){
                if(this.type=='new'){
                    let data={
                        ...
                        departmentid:this.bmxl,
                        employee1id:this.fzrxl,
                        memo:this.memo
                    };
                    console.log(data);
                    //新增
                    this.$post(this.$api.chukuad,data).then(data=>{
                        if(data.code==0){
                            this.$message({
                                message: '添加成功',
                                type: 'success'
                    ...
                    // 修改
                    console.log(data);
                    this.$post(this.$api.chukuedit,data).then(data=>{
                        if(data.code==0){
                            this.$message({
                                message: '修改成功',
                                type: 'success'
            ...
            onshenh(row){
                this.$confirm(' 是否确定审核?', '提示', {
                    ...
                    this.$post(this.$api.chukuauth,{chukuid:row.chukuid}).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                type: 'success',
            ...
            onylxdetermine(){
                ...
                if(this.mxtype=='new'){
                    ...
                    this.$post(this.$api.goodschukuadd,data).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                message: '添加成功',
                    ...
                    this.$post(this.$api.goodschukuedit,data).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                message: '修改成功',
            ...
            onmingxdelete(row){
                this.$confirm(' 是否确定删除?', '提示', {
                    ...
                    this.$post(this.$api.goodschukudel,{slaveid:row.slaveid}).then(data=>{
                        console.log(data);
                        if(data.code==0){
                            this.$message({
                                type: 'success',
